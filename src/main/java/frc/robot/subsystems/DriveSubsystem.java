...
public class DriveSubsystem extends SubsystemBase {
  ...
  // The gyro sensor
  // FIX NAVX CONFIGURATION NOW THAT IT'S VERTICAL
private final AHRS m_gyro = new AHRS(SerialPort.Port.kUSB);
  ...
  public void driveWithJoystick(double leftX, double leftY, double rightX, boolean fieldRelative) {
    ...
    final var xSpeed =
        m_xspeedLimiter.calculate(MathUtil.applyDeadband(leftX, 0.05))
    ...
    final var ySpeed =
        -m_yspeedLimiter.calculate(MathUtil.applyDeadband(leftY, 0.05))
            * DriveConstants.kMaxSpeed;
    ...
    final var rot =
        -m_rotLimiter.calculate(MathUtil.applyDeadband(rightX, 0.05))
